...
public class ExpressionEval {
			...
			if (token.equals("*") || token.equals("/")) {
				...
				if (token.equals("*")) {
					if (type.equals("FLOAT")) 
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.MULTF,op1,op2,result));
					else
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.MULTI,op1,op2,result));
				}
				else if (token.equals("/")) {
					if (type.equals("FLOAT")) 
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.DIVF,op1,op2,result));
					else
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.DIVI,op1,op2,result));
				}
			}
			else {
				stack.push(token);
	...
	private static String SimpAdd (String expr) {
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (token.equals("+") || token.equals("-")) {
				...
				if (token.equals("+")) {
					if (type.equals("FLOAT")) 
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.ADDF,op1,op2,result));
					else
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.ADDI,op1,op2,result));
				}
				else if (token.equals("-")) {
					if (type.equals("FLOAT")) 
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.SUBF,op1,op2,result));
					else
						SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.SUBI,op1,op2,result));
				}
	...
	public static List<String> checkOps(String op1, String op2) {
		...
		if (op1.indexOf('.') != -1 ) {
			...
			SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.STOREF,op1,null,reg));
			res1 = reg;
		}
		else if (isNumeric(op1)) {
			reg = GetNextReg("INT");
			SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.STOREI,op1,null,reg));
			res1 = reg;
		}
		if (op2.indexOf('.') != -1 ) {
			reg = GetNextReg("FLOAT");
			SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.STOREF,op2,null,reg));
			res2 = reg;
		}
		else if (isNumeric(op2)) {
			reg = GetNextReg("INT");
			SemanticHandler.currentIRList.add(new IRNode(IRNode.IROpcode.STOREI,op2,null,reg));
			res2 = reg;
		}
